...
try:
    ...
    if time_range and time_range[0] and time_range[1]:
        ...
        if len(date_range) == 2:
            ...
            partitions_query = "SELECT DISTINCT name FROM slurm_data.partitions ORDER BY name"
            ...
            with col2:
                ...
                jobs_df = conn.execute(jobs_query, selected_partitions if selected_partitions else []).df()
            ...
            partition_usage_df = conn.execute(partition_usage_query, selected_partitions if selected_partitions else []).df()
            ...
            if not partition_usage_df.empty:
                ...
                fig = px.bar(
                    ...
                    color='job_count',
                    labels={'total_cpus': 'Total CPUs Allocated', 'partition': 'Partition', 'job_count': 'Number of Jobs'},
                    color_continuous_scale=px.colors.sequential.Viridis
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("No resource utilization data available for the selected filters")
            ...
            duration_df = conn.execute(duration_query, selected_partitions if selected_partitions else []).df()
            ...
            if not duration_df.empty:
                ...
                fig = px.histogram(
                    ...
                    labels={'duration_minutes': 'Duration (minutes)', 'count': 'Number of Jobs'},
                    opacity=0.7
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
            ...
            with st.expander("View Raw Data"):
                ...
                LIMIT 100
                """
                jobs_raw_df = conn.execute(jobs_raw_query, selected_partitions if selected_partitions else []).df()
                st.dataframe(jobs_raw_df)
    else:
        st.warning("No job data available in the database. Please run the SLURM data pipeline first.")
        
except Exception as e:
    st.error(f"Error accessing database: {str(e)}")
    st.info("Please run the SLURM data pipeline first to collect data from your SLURM cluster.")
    
# Footer
st.markdown("---")
st.markdown("SLURM Monitoring Dashboard | Alfred Wegener Institute for Polar and Marine Research")
